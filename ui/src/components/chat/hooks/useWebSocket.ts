...
export function useWebSocket({
  ...
  const connectToWebSocket = useCallback(
    async (conversationId: string) => {
      ...
      try {
        ...
        console.log(
          `[WebSocket] Connecting to Socket.IO at ${socketUrl} with conversation ID: ${conversationId}`
        );
        ...
        if (
          ...
          console.log(
            `[WebSocket] Already connected to conversation ${conversationId}`
          );
        ...
        console.log(
          `[WebSocket] ${
            authToken
              ? "Using auth token for connection"
              : "No auth token available for connection"
          }`
        );
        ...
        if (socketRef.current) {
          console.log(
            "[WebSocket] Disconnecting previous Socket.IO connection"
          );
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log(
        `[WebSocket] Setting up connection for ID: ${conversationId} (first time)`
      );
    ...
    return () => {
      ...
      if (socketRef.current) {
        ...
        console.log("[WebSocket] Component unmounting, scheduling disconnect");
        ...
        if (!cleanupRegisteredRef.current.get(socket)) {
          ...
          setTimeout(() => {
            ...
            if (socketRef.current === socket) {
              console.log("[WebSocket] Disconnecting Socket.IO on unmount");
              socket.disconnect();
              socketRef.current = null;
              ...
              console.log(
                "[WebSocket] Socket was replaced, not disconnecting old one"
              );
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      console.log("[WebSocket] Component fully unmounting");
