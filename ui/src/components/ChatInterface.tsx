...
export function ChatInterface({
    ...
    async (id: string) => {
      try {
        ...
        if (result) {
          ...
          console.log(
            "[ChatInterface] Successfully restored conversation state:",
            {
              messages: result.userMessages.length,
              workflows: Object.keys(result.messageWorkflows).length,
            }
          );
        } else {
  ...
  const finalizeConversationMessages = useCallback(async () => {
    ...
    if (!success) {
      ...
      console.log(
        "[ChatInterface] Successfully finalized conversation messages"
      );
    }
  ...
  useEffect(() => {
    if (currentPhase === "completed") {
      console.log(
        "[ChatInterface] Phase is completed, finalizing conversation messages"
      );
  ...
  const { socket, isConnected, joinConversation, reconnect } = useWebSocket();

  const handleSubmit = async (e?: React.FormEvent) => {
    e?.preventDefault();
    if (!inputValue.trim()) return;

    console.log(`[ChatInterface] Submitting user input: ${inputValue}`);
  ...
  const handleInitialMessage = async () => {
    ...
    console.log(
      `[ChatInterface] Processing initial message: ${initialMessage}`
    );
  ...
  useEffect(() => {
    ...
    initialConnectionTimeoutRef.current = setTimeout(() => {
      if (!isConnected && conversationId) {
        console.log(
          "[ChatInterface] Connection not established after timeout, rejoining conversation"
        );
        reconnect();
        setTimeout(() => joinConversation(conversationId), 1000);
      }
    }, 5000);
  ...
  }, [conversationId, joinConversation, isConnected, reconnect]);
  ...
  useEffect(() => {
    ...
    connectionCheckIntervalRef.current = setInterval(() => {
      ...
      if (
        ...
        console.log(
          `[ChatInterface] No userMessages received for ${Math.round(
    ...
    addHandler("connected", (data) => {
      console.log("[ChatInterface] WebSocket connected successfully", data);
    ...
    addHandler("pong", () => {
      console.log("[ChatInterface] Received pong from server");
      setLastMessageTime(Date.now());
    });
    ...
    const pingInterval = setInterval(() => {
      if (socket && isConnected) {
        console.log("[ChatInterface] Sending ping to server");
  ...
  useEffect(() => {
    if (!propConversationId && !conversationId) {
      ...
      console.log(
        `[ChatInterface] Creating new conversation with ID: ${newId}`
      );
      setConversationId(newId);
    } else if (propConversationId && propConversationId !== conversationId) {
      console.log(
        `[ChatInterface] Using provided conversation ID: ${propConversationId}`
      );
  ...
  const handleRestartAPIServer = useCallback(() => {
    ...
    console.log("[Debug] Attempting to restart API server connection");
  ...
  const handleAgentUpdate = (data: AgentUpdate) => {
    console.log(`[WebSocket] Handling agent update: ${JSON.stringify(data)}`);
    ...
    if (data.type === "discovery_plan_generated" && data.details?.plan) {
      state = AgentState.PLANNING;
      console.log(
        "[WebSocket] Processing discovery plan data:",
        data.details.plan
      );
    ...
    if (data.type === "plan_generated" && data.details?.plan) {
      state = AgentState.PLANNING;
      console.log("[WebSocket] Processing plan data:", data.details.plan);
    ...
    if (data.type === "step_about_to_execute" && data.details) {
      console.log("[WebSocket] Processing step_about_to_execute event:", data);
      ...
      if (stepId) {
        ...
        console.log("[WebSocket] Added new execution step:", {
          compositeKey,
    ...
    if (data.type === "step_update" && data.data) {
      console.log("[WebSocket] Processing step_update event:", data);
      ...
      if (stepData.approval_required && stepData.step_id) {
        console.log("[WebSocket] Step requires approval:", stepData.step_id);
    ...
    if (data.type === "approval_required" && data.details) {
      console.log("[WebSocket] Processing approval_required event:", data);
    ...
    if (data.type === "verification_complete" && data.details) {
      ...
      if (data.details?.status) {
        setValidationStatus(
          data.details.status as "success" | "failure" | "partial"
        );
      }
    ...
    if (data.type === "response_complete") {
      console.log("[WebSocket] Processing response_complete event");
      ...
      setTimeout(() => {
        console.log("[WebSocket] Finalizing workflow after response_complete");
        setIsAgentResponding(false);
  ...
  const processMessage = async (message: string) => {
    ...
    console.log("[ChatInterface] Chat history to send:", chatHistoryString);

    if (conversationId) {
      console.log(
        `[ChatInterface] Using existing conversation ID: ${conversationId}`
      );

      try {
        const response = await createConversation(conversationId);
        console.log(
          `[ChatInterface] Successfully registered conversation with server: ${response.id}`
        );
  ...
  const handleApproveStep = useCallback(
    (stepId: string) => {
      console.log("[ChatInterface] Approving step:", stepId);
      if (!socket || !isConnected || !conversationId) {
        console.log("[ChatInterface] Cannot approve - no active connection");
        return;
      }
      ...
      console.log(
        "[ChatInterface] Using original step_id for approval:",
        originalStepId
      );
  ...
  const handleRejectStep = useCallback(
    (stepId: string) => {
      console.log("[ChatInterface] Rejecting step:", stepId);
      if (!socket || !isConnected || !conversationId) {
        console.log("[ChatInterface] Cannot reject - no active connection");
        return;
      }
      ...
      const originalStepId = step.original_step_id;
      console.log(
        "[ChatInterface] Using original step_id for rejection:",
        originalStepId
      );
  ...
  const handleCancelResponse = useCallback(() => {
    if (!socket || !isConnected || !conversationId) {
      console.log("[ChatInterface] Cannot cancel - no active connection");
      return;
    }

    console.log("[ChatInterface] Cancelling current response");
