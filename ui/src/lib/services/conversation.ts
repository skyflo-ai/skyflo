...
export async function fetchConversationDetails(
  ...
  try {
    ...
    if (response.ok && data.status === "success" && data.messages) {
      ...
      console.log(
        "[ConversationService] Successfully processed conversation state:",
        {
          messages: processedMessages.length,
          workflows: Object.keys(loadedWorkflows).length,
        }
      );
...
export async function finalizeConversationMessages(
  ...
  for (let i = 0; i < userMessages.length; i++) {
    ...
    if (messageWorkflows[i]) {
      console.log(
        "[ConversationService] Adding workflow message:",
        messageWorkflows[i]
      );
      formattedMessages.push({
        role: "sky",
  ...
  console.log("[ConversationService] Formatted messages:", formattedMessages);
  console.log(
    "[ConversationService] Final response value being saved:",
    finalResponse
  );
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.log(
        "[ConversationService] Successfully updated conversation messages"
      );
      return true;
    }
  } catch (error) {
