...
# Alternative config for src.api import structure
SRC_TORTOISE_ORM_CONFIG = {
    "connections": {"default": str(settings.DATABASE_URL)},
    "apps": {
        "models": {
            "models": [
                "src.api.domain.models.user",
                "src.api.domain.models.conversation",
                "aerich.models",
            ],
            "default_connection": "default",
        }
...
async def init_db() -> None:
    ...
    try:
        ...
        try:
            ...
            logger.info("Falling back to src.api import structure")
            await Tortoise.init(config=SRC_TORTOISE_ORM_CONFIG)
