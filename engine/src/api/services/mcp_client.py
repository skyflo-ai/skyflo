...
class MCPClient:
    ...
    def __init__(
        ...
        if event_callback is not None:
            ...
            logger.info(f"Registered event_callback in MCPClient constructor")
        else:
            logger.info(f"MCPClient initialized with None callback")
    ...
    async def get_tools(self, category: Optional[str] = None) -> Dict[str, Any]:
        ...
        logger.info(f"Fetching tools from endpoint: {self.base_url}{endpoint}")
        ...
        try:
            async with self.session.get(f"{self.base_url}{endpoint}") as response:
                ...
                logger.info(f"Successfully fetched {len(tools_data['tools'])} tools")
                return tools_data
