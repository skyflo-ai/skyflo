...
class ExecutorAgent(BaseAgent):
    ...
    def __init__(
        ...
        system_message = system_message or settings.EXECUTOR_SYSTEM_MESSAGE
        super().__init__(
            name=name,
    ...
    async def _summarize_execution_state(
        ...
        logger.info(f"Summarizing execution state due to token limit being exceeded")
    ...
    async def execute_plan(self, plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"Starting plan execution with ID: {plan.get('plan_id')}")
        ...
        logger.info(f"User query in model_context: {self._model_context.get('user_query')}")
        ...
        try:
            ...
            for step in steps:
                try:
                    ...
                    logger.info(f"Current token count before step {step_id}: {current_token_count}")
                    ...
                    if step.get("recursive", False):
                        logger.info(f"Step {step_id} is marked as recursive")
                        logger.debug(
    ...
    async def _execute_recursive_step(
        ...
        logger.info(f"Starting recursive execution for step {step.get('step_id')}")
        ...
        try:
            ...
            if not param_resources:
                ...
                for resource in resources:
                    ...
                    logger.info(f"Executing recursive step for {iterate_param}={resource}")
...
Return a JSON array of tool calls that follow the same structure as the current step.""",
        ...
        try:
            ...
            if json_start != -1 and json_end > 0:
                ...
                logger.info(f"Generated {len(tool_calls)} recursive tool calls")
                return tool_calls
        except Exception as e:
    ...
    async def _execute_step(self, step: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Executing step {step_id}: {tool} {action}")
            ...
            if approval_required:
                logger.info(f"Step {step_id} requires approval")
                if hasattr(self, "event_callback") and self.event_callback:
                    await self.event_callback(
                        {
                    ...
                    logger.info(f"Step {step_id} was approved")
                    await asyncio.sleep(0.5)
            ...
            if action.lower() in ["logs", "log", "get_logs"]:
                ...
                logger.info(f"Preprocessed log request parameters for step {step_id}")
            ...
            # Basic validation for Kubernetes resource types
                ...
                try:
                    ...
                    logger.info(f"Correcting invalid resource type in step {step_id}")
                    if "pods" in str(e):
                    ...
                    logger.info(f"Resource type corrected for step {step_id}")
            ...
            logger.info(f"Calling tool {tool} with action {action}")
            ...
            logger.info(f"Step {step_id} execution completed")
    ...
    async def _resolve_dynamic_parameters(
        ...
        try:
            ...
            logger.info(f"Found placeholders to resolve: {list(placeholder_params.keys())}")
            ...
            for param_name, resolved_value in resolved_resource_params.items():
                if resolved_value is not None:  # Only update if we got a valid resolution
                    ...
                    logger.info(f"Resolved parameter {param_name} to: {resolved_value}")
    ...
    async def _resolve_resource_references(
        ...
        logger.info(f"Built resource context with {len(resource_context)} resource listings")
        ...
        try:
            ...
            if json_start != -1 and json_end > 0:
                ...
                logger.info(f"LLM resolved resources: {resolution_result}")
                    ...
                    if param_name in placeholder_params:
                        ...
                        if (
                            ...
                            if isinstance(resolved_value, str) and "," in resolved_value:
                                ...
                                logger.info(f"Split multi-resource {param_name} into: {resources}")
    ...
    def _extract_resource_names_from_output(
        ...
        logger.info(f"Extracted {len(resource_names)} {resource_type} names: {resource_names}")
        return resource_names
    ...
    async def _extract_pod_names_from_previous_results(
        ...
        logger.info(f"execution_state type: {type(execution_state)}")
        logger.info(f"user_query type: {type(user_query)}")
        logger.info(f"execution_state: {execution_state}")
        ...
        try:
            ...
            for step_key, step_result in execution_state.items():
                ...
                if step_key.startswith("step_"):
                    logger.info(f"Building context for step: {step_key}: {step_result}")
                    context_info.append(
                        {
                            "step_id": step_key.replace("step_", ""),
            ...
            logger.info(f"Context info in _extract_pod_names_from_previous_results: {context_info}")
            ...
            if pod_names_response:
                ...
                logger.info(f"LLM identified relevant pod names: {pod_names}")
            ...
            for step_key, step_result in execution_state.items():
                ...
                if (
                    ...
                    logger.info(f"Falling back to processing all pods from namespace: {namespace}")
        ...
        # Log what we found
        if pod_names:
            logger.info(f"Extracted pod names: {pod_names}")
        else:
    ...
    async def _preprocess_log_request(
        ...
        logger.info(f"Preprocessing log request with parameters: {parameters}")
        ...
        if "resource_type" in processed_params and processed_params["resource_type"] != "pods":
            ...
            logger.info(f"Corrected resource_type from '{original_type}' to 'pods' for log request")
                ...
                if keyword in user_query.lower():
                    ...
                    if keyword in words:
                        ...
                        if idx + 1 < len(words):
                            ...
                            logger.info(f"Extracted namespace '{namespace}' from query")
                            break

            # Default to "default" namespace if not specified
            if "namespace" not in processed_params:
                processed_params["namespace"] = "default"
                logger.info("Using default namespace for log request")
